    ...
    <div className="bg-white rounded-xl shadow-lg border border-brand-coral/20 p-3 sm:p-4 md:p-6">
      <div className="flex flex-col space-y-4">
        ...
        <div className="relative">
          <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-brand-gray" size={16} />
          <input
            type="text"
            placeholder={placeholder}
            value={searchTerm}
            onChange={(e) => onSearchChange(e.target.value)}
            className="w-full pl-9 pr-4 py-2 text-sm sm:text-base border border-brand-gray/30 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-red focus:border-brand-red transition-colors"
          />
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between space-y-2 sm:space-y-0">
          <button 
            ...
            className="flex items-center space-x-2 text-brand-blue hover:text-brand-red transition-colors text-sm sm:text-base"
          >
          ...
          {hasActiveFilters && (
            <div className="flex flex-col sm:flex-row sm:items-center space-y-1 sm:space-y-0 sm:space-x-2">
              ...
              <button
                ...
                className="text-brand-red hover:text-brand-red-dark transition-colors flex items-center space-x-1 text-xs sm:text-sm"
              >
